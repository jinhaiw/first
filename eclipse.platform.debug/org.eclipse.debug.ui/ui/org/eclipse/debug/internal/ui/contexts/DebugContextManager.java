	...
	protected IDebugContextService createService(IWorkbenchWindow window) {
		...
		if (service == null) {
			...
			if (window.getShell() == null || evaluationService == null) {
				...
				for (Object iDebugContextListener : fGlobalListeners.getListeners()) {
					if (iDebugContextListener instanceof IDebugContextListener) {
						service.addDebugContextListener((IDebugContextListener) iDebugContextListener);
					}
				}
			}
		}
